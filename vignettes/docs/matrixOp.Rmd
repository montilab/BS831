...
In fact, notice that the gain in efficiency of the `%*%` over `apply` is largely lost when computing the column sum, since there are only 10 columns, hence only ten calls of the `sum` function. The gain in efficient with respect to the `for` loop is still there.
