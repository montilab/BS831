...
(mean 0 and stdev 1); visualize it as a boxplot; generate a second
sample (Y) of points that are linearly related to the first sample.
...
where most of the density lies. Furthermore, I needed to know where
most of a distribution mass lies. Otherwise, I might end up with a
truncated plot.

```{r plot2}
X <- seq(-4,4,.1)
plot(X,dnorm(X,mean=0,sd=3),pch=20)
plot(X,dnorm(X,mean=3,sd=1),pch=20)
```
