...
library(circlize)
library(ggplot2)
library(vennr) # see github.com/montilab/vennr
```
...
We start by uploading a Breast Cancer dataset already restricted to only two tumor categories, _basal_ and _nonbasal_, described in [[Richardson et al., PNAS 2006](http://europepmc.org/article/MED/16473279)].

```{r load.data}
## load the ExpressionSet object
data(renamedBreastDB)
cancerSet <- renamedBreastDB
head(pData(cancerSet))
table(cancerSet$diseaseState)
```

We have extracted the data of interest into the object `cancerSet`. We are now ready to use the `t.test` function. We first show its use applied to a single gene.
...
Here's an aside on efficient computation (see the module [Efficient Computation in R](https://montilab.github.io/BS831/articles/docs/matrixOp.html)). Above, we used the function `t.test` together with `apply` to compute the t statistics and p-values, which requires calling the `t.test` function `r nrow(cancerSet)` times. Below, we show the use of the function `mt.teststat` in the package [`multtest`](https://bioconductor.org/packages/release/bioc/html/multtest.html), to compute the t-statistics for a large number of genes more efficiently. 
...
all.equal(res_slow, -res_fast)
```

Notice the considerable speed-up achieved. 
```{r}
## show execution times
rbind('t.test' = t_slow, multtest = t_fast)
```
...
print(unique(design1)) # show the 'coding' for the two classes
## 3. Fitting the model
...
limmaRes1 <- topTable(fit1, coef="basal",adjust.method = "BH", n = Inf, sort.by = "P")
```
...
print(unique(design2)) # show the 'coding' for the two classes
## 2. Definition of the "contrast"
...
## 6. extract the relevant differential analysis information, sorted by p-value
limmaRes2 <- topTable(fit2, adjust.method = "BH", n = Inf, sort.by = "P")
```
