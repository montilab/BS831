...
colnames(design)
## simplify names
colnames(design) <- c("intercept", "basal")
...
limmaRes1 <- topTable(fit, coef="basal",adjust.method = "BH", n = Inf, sort.by = "P")
```
...
print(unique(design)) # show the 'coding' for the two classes
## 2. Definition of the "contrast"
contrast.matrix <- makeContrasts(Basal-LumA, levels = design)
## 3. Fitting the model
fit <- lmFit(cancerSet, design)
